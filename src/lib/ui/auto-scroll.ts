...
export function createAutoScroller(opts: AutoScrollOpts): AutoScroller {
	...
	function tick() {
		...
		if (opts.axis === 'vertical') {
			...
			else if (p > rect.bottom - edgeZone)
				delta = edgeStep(p - (rect.bottom - edgeZone), edgeZone, maxStep);
			if (delta !== 0) scroller.scrollTop += delta;
		} else {
			if (p < rect.left + edgeZone) delta = -edgeStep(rect.left + edgeZone - p, edgeZone, maxStep);
			else if (p > rect.right - edgeZone)
				delta = edgeStep(p - (rect.right - edgeZone), edgeZone, maxStep);
